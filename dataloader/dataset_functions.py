...
def __getitem__(sample, 
    ...
    sliced = slice_audio(sample, slice_seconds=1.07) # This way you keep 1 second after last cut
    localization = get_localization(hrtf, label)
    # Add ramping -- currently inside transform
    transform = transform(sliced, localization, n=0.035, target_samplerate=target_samplerate) # cut first and last 35ms from audio
    cochleagram = generate_cochleagram(transform, target_samplerate)
