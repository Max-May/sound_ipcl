...
class IPCL(nn.Module):
    def __init__(
        ...
        """
        base_encoder: backbone that encodes cochleagrams
        numTrainFiles: total number of audio files in training dataset
        ...
        """
        super(IPCL, self).__init__()

        if device is None:
            device = "cuda" if torch.cuda.is_available() else "cpu"

        self.base_encoder = base_encoder
        self.numTrainFiles = numTrainFiles
        self.K = K
        self.T = T
        ...
        self.criterion = NCECriterion(numTrainFiles)
    ...
    def forward(self, x, y):
        ...
        if Z < 0:
            if self.distributed:
                ...
                Z = (out_.mean() * self.numTrainFiles).detach()
            else:
                Z = (out.mean() * self.numTrainFiles).detach()
            self.Z = Z
